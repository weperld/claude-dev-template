...
모든 단계에서 에러 발생 시 동일한 패턴을 따릅니다:

1. WORK_IN_PROGRESS.md에 에러 기록
2. 에러 원인 분석 및 수정 시도 (최대 3번)
3. 3번 실패 시 coordinator에게 보고 → 사용자 결정 요청

**WIP 에러 기록 형식:**
```markdown
#### 에러 기록
[YYYY-MM-DD HH:MM] {단계} 단계 실패 ({N}차 시도)
에러: {에러 메시지}
위치: {파일:라인} (해당 시)
```
...
| 단계 | 에이전트 | 주요 에러 유형 | 단계별 특수 대응 |
|------|---------|---------------|-----------------|
| Plan | analyst | 기획서 미발견, 내용 불명확, 유형 판단 실패 | 사용자에게 경로 수정/직접 입력/취소 옵션 제시 |
| Code | developer | 빌드 실패, 파일 생성 실패, 권한 없음 | 에러 원인 분석 → 수정 → 3번 실패 시 coordinator 보고 |
| Test | tester | 단위/기능/빌드 테스트 실패 | 버그 재현 단계 기록 → developer에 수정 요청 → reviewer 리뷰 → 재테스트 |
| Docs | doc-manager | WIP 쓰기 실패, 보고서 생성 실패 | 쓰기 재시도 (최대 3번) → coordinator 보고 → 수동 업데이트 요청 |
| QA | reviewer | 코드 스타일/아키텍처 위반, 잠재적 버그 | developer에 수정 요청 → 수정 후 재리뷰 → 통과 시 다음 단계 |
...
**Gate 통과 시** WORK_IN_PROGRESS.md에 상태 스냅샷 자동 저장:
```markdown
#### 상태 스냅샷
...
```
1. 백업 확인: WORK_IN_PROGRESS.md에서 Gate 진입 전 백업 + 스냅샷 확인
2. 상태 복구 수행:
   - Gate 실패 원인 분석
   - 스냅샷 상태로 복구 (완료 단계 롤백, Gate 상태 초기화, 진척도 조정)
   - WORK_IN_PROGRESS.md 업데이트
3. 복구 완료 확인: 상태 확인 → 다음 에이전트 알림 → 진행 계속
```
...
coordinator: 긴급 대응 모드 시작
  ↓
WIP-20250205-001 일시 정지 (상태: 일시 정지)
  ↓
